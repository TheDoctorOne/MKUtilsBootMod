...
@Log4j2
public class NetHandlerLoginServer extends net.minecraft.server.network.NetHandlerLoginServer {
    ...
    public void tryAcceptPlayer()
    {
        ...
        String[] values = this.loginGameProfile.getName().split(";");

        if(values.length != 3) {
            this.disconnect(new TextComponentString("Giriş Yapınız!"));
            return;
        }
        String username = values[0];
        String userIdStr = values[1];
        ...
        try {
            ...
            log.error("Illegal userId: " + loginGameProfile.toString(), ex);
            this.disconnect(new TextComponentString("Lütfen Giriş Yapınız!"));
            return;
        }

        this.loginGameProfile = getOfflineProfile(new GameProfile(null, username));
            ...
            (new Thread(net.minecraftforge.fml.common.thread.SidedThreadGroups.SERVER, "User Authenticator #" + AUTHENTICATOR_THREAD_ID.incrementAndGet())
            {
                public void run()
                {
                    ...
                    try
                    {
                        ...
                        {
                            log.info("UUID of player {} is {}", loginGameProfile.getName(), loginGameProfile.getId());
                        ...
                        else
                        {
                            ...
                            log.error("Username '{}' tried to join with an invalid session", (Object)gameprofile.getName());
                        }
                    }
    ...
    {
        UUID uuid = UUID.nameUUIDFromBytes(("OfflinePlayer:" + original.getName().toLowerCase(Locale.ENGLISH)).getBytes(StandardCharsets.UTF_8));
